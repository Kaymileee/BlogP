...
namespace Blog.Services
{
    public class PostService : IPostService
    {
        ...
        private async Task ProcessTag(PostCreateRequest request, Post post)
        {
            if (request.ListTag != null)
            {
                foreach (var labelText in request.ListTag)
                {
                    ...
                    var tagSlug = TextHelper.ToUnsignedString(labelText.ToString());
                    var existingLabel = await _context.Tags.FindAsync(tagSlug);
                    ...
                    if (await _context.PostTags.FindAsync(post.PostId, tagSlug) == null)
                    {
        ...
        public async Task SeedData(List<PostCreateRequest> requests)
        {
