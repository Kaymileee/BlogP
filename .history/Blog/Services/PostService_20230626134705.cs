...
namespace Blog.Services
{
  public class PostService : IPostService
  {
    ...
    public static List<string> GetRandomStrings(List<string> strings)
    {
      ...
      int count = random.Next(1, 6); // Tạo một số nguyên ngẫu nhiên từ 1 đến 5
      List<string> randomStrings = new List<string>();
      for (int i = 0; i < count; i++)
      {
        int index = random.Next(strings.Count);
        randomStrings.Add(strings[index]);
      }
      return randomStrings;
    }

    private async Task ProcessPostCategory(int postId, List<string> listcategoryId)
    {
      listcategoryId = GetRandomStrings(listcategoryId);
      foreach (string categoryId in listcategoryId)
      {
        var pc = await _context.PostCategories.FindAsync(postId, categoryId);
        if (pc == null)
        {
    ...
    public async Task SeedData(List<PostCreateRequest> requests, Guid? userId, List<string> listcategoryId)
    {
      foreach (var request in requests)
      {
        ...
        await ProcessPostCategory(post.PostId, listcategoryId);
        await _context.SaveChangesAsync();
      }
    }
  }
